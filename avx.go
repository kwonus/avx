...
func slash(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "text/html")
	io.WriteString(w, "<h2>This is the AVX web-server by AV Text Ministries.</h2><br>" +
...
func main() {	// Arguments SDK_DIR  CSS_DIR
	...
	key := uint16(1)
	cnt := readByte(flx)					// search_cnt
	buffer := make([]byte, 64)

	for n := cnt+1; n > 1; n = cnt+1 {
		flx.Read(buffer[0:cnt:cnt])			// search
		buffer[cnt] = 0						// null-terminate
		word := string(buffer[0:n:n])
		lex[key] = word

		cnt = readByte(flx)					// display_cnt
		if cnt > 0 {
			n = cnt+1
			flx.Read(buffer[0:cnt:cnt])		// display
			buffer[cnt] = 0 				// null-terminate
			word = string(buffer[0:n:n])
		}
		modern := word
		...
		cnt = readByte(flx)					// modern_cnt
		if cnt > 0 {
			n = cnt+1
			flx.Read(buffer[0:cnt:cnt])		// modern
			buffer[cnt] = 0 				// null-terminate
			modern = string(buffer[0:n:n])
		...
		if (diff) {
			modern = strings.ToLower(modern)
			word = strings.ToLower(word)
			diff = (modern != word)
		}

		if (pn > 0) && diff {
			decor[key] = decoratePN(pos, pn)
		}
		cnt = readByte(flx)						// search_cnt
		key++;
	}
